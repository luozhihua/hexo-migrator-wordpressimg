var xml2js = require('xml2js'),
    ...
    file = require('fs'),
    path = require('path'),
    imgdl = require('./img-download'),
    map301 = require('./map301');

var captialize = function(str){
  return str[0].toUpperCase() + str.substring(1);
};

hexo.extend.migrator.register('wordpressimg', function(args, callback){
  var source = args._.shift();
  var host = args._.shift();
  var list301 = {}; // 301重定向映射关系

  if (!source || !host){
    var help = [
      'Usage: hexo migrate wordpress <source> <wordpress_host>',
      '',
  ...
  async.waterfall([
    ...
    function(xml, next){
      ...
      async.each(xml.rss.channel[0].item, function(item, next){
        ...
        imgdl(data, host, function(newdata) {
          post.create(newdata, next);
        });
    ...
    function(next) {
      var filepath = path.join(__dirname, '../../301.json');
      file.writeFile(filepath, JSON.stringify(list301, 4, 4));
      next();
    }
  ], callback);
});
